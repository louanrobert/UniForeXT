...
var EXPLORED_BORDER_COLOR = "#33a02c"; // ColorBrewer Paired green for explored nodes
var EXPLORED_BORDER_WIDTH = 3;

function styleNavigationButtons(container) {
  if (!container) return;

  var nav = container.querySelector(".vis-navigation");
  if (nav) {
  ...
  var buttons = container.querySelectorAll(".vis-button");
  for (var i = 0; i < buttons.length; i++) {
    ...
    btn.style.border = "1px solid hsl(var(--border) / 0.25)";
    ...
    btn.onmouseenter = function () {
      ...
      this.style.boxShadow = "0 6px 14px rgba(0, 0, 0, 0.32)";
    };
  ...
  // Keep only zoom controls for a cleaner UI.
  var directional = container.querySelectorAll(
    ".vis-button.vis-up, .vis-button.vis-down, .vis-button.vis-left, .vis-button.vis-right",
  );
  for (var j = 0; j < directional.length; j++) {
    directional[j].style.display = "none";
  }

  var zoomIn = container.querySelector(".vis-button.vis-zoomIn");
  var zoomOut = container.querySelector(".vis-button.vis-zoomOut");
  var fit = container.querySelector(".vis-button.vis-zoomExtends");

  if (zoomIn) {
    zoomIn.innerHTML = '<i data-lucide="zoom-in" class="w-4 h-4"></i>';
    zoomIn.style.fontSize = "0";
  }
  if (zoomOut) {
    zoomOut.innerHTML = '<i data-lucide="zoom-out" class="w-4 h-4"></i>';
    zoomOut.style.fontSize = "0";
  }
  if (fit) {
    fit.innerHTML = '<i data-lucide="maximize" class="w-4 h-4"></i>';
    fit.style.fontSize = "0";
  }
...
function initGraph(uri) {
  ...
  try {
    ...
    styleNavigationButtons(container);
    // Navigation buttons are created asynchronously by vis.js; retry a couple of times,
    // but avoid subtree observers because tooltip mutations on hover can cause re-style loops.
    setTimeout(function () {
      styleNavigationButtons(container);
    }, 120);
    setTimeout(function () {
    ...
    network.on("beforeDrawing", function (ctx) {
      ...
      ctx.save(); // Prevent canvas state leak into vis.js node rendering
      ...
      ctx.restore(); // Restore canvas state so vis.js renders nodes correctly
    });
...
function normalizeNode(node) {
  // Extract and store type from title (format: "TypeName\nURI")
  if (node.title && !node._nodeType) {
    var nlIdx = node.title.indexOf("\n");
    node._nodeType = nlIdx >= 0 ? node.title.substring(0, nlIdx) : null;
  }
  // Ensure non-literal nodes have complete color specs for vis.js
  if (node.shape === "box" || (node.id && node.id.indexOf("_lit_") >= 0)) return node;

  // Normalize color to ensure it has background, border, highlight, and hover states
  var c = node.color;
  if (c && typeof c === "object") {
    var bg = c.background || "#a6cee3";
    var bd = c.border || "#666";
    if (!c.highlight) {
      c.highlight = { background: bg, border: bd };
    }
    if (!c.hover) {
      c.hover = { background: bg, border: bd };
    }
    ...
    node.color = {
      background: "#a6cee3",
      border: "#a6cee3",
      highlight: { background: "#a6cee3", border: "#a6cee3" },
      hover: { background: "#a6cee3", border: "#a6cee3" },
    };
  }
  // Ensure font color is set for dark theme
  if (!node.font || !node.font.color) {
    node.font = Object.assign({ color: "#e0e0e0", size: 12 }, node.font || {});
  }
  return node;
}
...
function applyExpansionData(nodeId, data) {
  ...
  data.nodes.forEach(function (node) {
    if (!existingNodeIds.has(node.id)) {
      newNodes.push(normalizeNode(node));
    }
  });
...
function showExpansionDialog(nodeId, summary) {
  ...
  var html =
    '<h3 class="text-foreground mb-1 text-sm font-semibold">Expand "' +
    escapeHtml(nodeLabel) +
    '"</h3>';
  html +=
    '<p class="text-muted-foreground mb-3 text-xs">' + summary.totalCount + " neighbors found</p>";
  html += '<div class="mb-1.5 text-xs text-muted-foreground">Select types to load:</div>';

  summary.types.sort(function (a, b) {
  ...
  for (var i = 0; i < summary.types.length; i++) {
    ...
    html +=
      '<label class="flex items-center gap-2 px-2 py-1 rounded-md mb-0.5 cursor-pointer text-xs hover:bg-accent transition-colors">';
    html +=
      '<input type="checkbox" class="type-check" value="' +
      escapeHtml(t.type) +
      '"' +
      defaultChecked +
      ">";
    html +=
      '<span class="type-dot w-3 h-3 rounded-full shrink-0 inline-block" style="background:' +
      t.color +
      '"></span>';
  ...
  if (summary.literalCount > 0) {
    ...
    html +=
      '<span class="type-dot w-3 h-3 rounded-full shrink-0 inline-block" style="background:#f0f0f0;border:1px solid #666"></span>';
    html += "<span>Data properties</span>";
    html +=
      '<span class="ml-auto text-muted-foreground text-xs">(' + summary.literalCount + ")</span>";
    html += "</label>";
  }
  ...
  html +=
    '<input type="number" class="px-2 py-1 bg-background border border-border rounded-md text-foreground text-xs outline-none focus:border-ring select-text w-[60px]" id="max-per-type" value="25" min="0" title="0 = no limit">';
  html += '<span class="text-xs text-muted-foreground ml-1">(0 = all)</span>';
  html += "</div>";

  html += '<div class="flex gap-2 mt-4 justify-end">';
  html +=
    '<button class="px-3 py-1.5 rounded-md border border-border bg-background text-foreground text-xs font-medium transition hover:bg-secondary hover:border-border/80 cursor-pointer" onclick="selectAllTypes(true)">All</button>';
  html +=
    '<button class="px-3 py-1.5 rounded-md border border-border bg-background text-foreground text-xs font-medium transition hover:bg-secondary hover:border-border/80 cursor-pointer" onclick="selectAllTypes(false)">None</button>';
  html +=
    '<button class="px-3 py-1.5 rounded-md border border-accent bg-accent text-accent-foreground text-xs font-medium transition hover:bg-accent/90 hover:border-accent/80 cursor-pointer" onclick="confirmExpansion()">Expand</button>';
  html +=
    '<button class="px-3 py-1.5 rounded-md border border-border bg-background text-foreground text-xs font-medium transition hover:bg-secondary hover:border-border/80 cursor-pointer" onclick="cancelExpansion()">Cancel</button>';
  html += "</div>";
...
function showNodeInfo(nodeId) {
  ...
  var html =
    '<h4 class="text-foreground mb-1 font-semibold">' + escapeHtml(node.label || nodeId) + "</h4>";
  if (node.title) {
    html +=
  ...
  if (!isLiteral) {
    ...
    html +=
      ...
      (isExplored ? "text-green-600" : "text-orange-500") +
      '">' +
      (isExplored
        ? '<i data-lucide="check"/> Explored'
        : '<i data-lucide="circle"/> Not explored (double-click to expand)') +
      "</div>";
  }
  info.innerHTML = html;
}
...
function expandLiteral(nodeId) {
  javaBridge.log("Expanding literal node: " + nodeId);
  var node = nodesDataSet.get(nodeId);
  if (!node) return;
...
function collapseLiteral() {
  ...
  if (node && node._shortLabel) {
    ...
    nodesDataSet.update({ id: expandedLiteralId });
    // Remove private keys via remove+re-add to avoid DataSet corruption
    var clean = Object.assign({}, nodesDataSet.get(expandedLiteralId));
    delete clean._shortLabel;
    delete clean.widthConstraint;
    nodesDataSet.update(clean);
  }
...
function updateLegend() {
  ...
  if (nodesDataSet) {
    nodesDataSet.forEach(function (node) {
      ...
      var type = node._nodeType;
      if (!type && node.title) {
        var nlIdx = node.title.indexOf("\n");
        type = nlIdx >= 0 ? node.title.substring(0, nlIdx) : null;
      }
      if (type && !types[type]) {
        var color = "#a6cee3";
        if (node.color) {
  ...
  var body = "";
  body +=
    '<div class="flex items-center gap-1.5 mb-1"><span class="w-3 h-3 rounded-full inline-block" style="background:#ccc;border:2px solid #33a02c"></span> Explored</div>';
  body +=
    '<div class="flex items-center gap-1.5 mb-1"><span class="w-3 h-3 rounded-full inline-block" style="background:#ccc;border:2px solid #999"></span> Not explored</div>';
  body += '<hr class="border-secondary my-1.5">';
  for (var type in types) {
    body +=
      '<div class="flex items-center gap-1.5 mb-1 whitespace-nowrap overflow-hidden text-ellipsis"><span class="w-3 h-3 rounded-full shrink-0 inline-block" style="background:' +
      types[type] +
      '"></span> ' +
      escapeHtml(type) +
      "</div>";
  ...
  legend.innerHTML =
    '<div class="px-3 py-2 bg-secondary text-secondary-foreground text-xs font-semibold cursor-pointer select-none flex items-center gap-1.5 hover:bg-accent transition-colors" id="legend-header" onclick="toggleLegend()"><span class="toggle-arrow flex text-[10px] transition-transform duration-200"><i data-lucide="chevron-down"></i></span> Legend</div>' +
    '<div class="px-3 py-2 overflow-hidden transition-all duration-250" id="legend-body" style="max-height:500px;opacity:1">' +
    body +
    "</div>";
  if (wasCollapsed) {
    ...
    legendBody.style.opacity = "0";
    legendBody.style.paddingTop = "0";
    legendBody.style.paddingBottom = "0";
    var arrow = legend.querySelector(".toggle-arrow");
    if (arrow) {
...
function toggleLegend() {
  ...
  if (legendBody) {
    if (collapsed) {
      ...
      legendBody.style.maxHeight = "500px";
      legendBody.style.opacity = "1";
      legendBody.style.paddingTop = "";
      legendBody.style.paddingBottom = "";
    }
  }
